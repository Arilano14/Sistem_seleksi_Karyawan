...
public function index()
{
    ...
    $keterangan = 'Halaman ini merupakan tahap penilaian alternatif terhadap sub kriteria berdasarkan metode AHP. Pastikan semua data sudah lengkap sebelum melanjutkan ke perhitungan.';
    ...
    // Ambil data alternatif & penilaian
    $alternatif = \App\Models\Alternatif::all();
    $penilaian = \App\Models\Penilaian::with('subKriteria')->get();

    // Buat array nilaiAlternatif[alternatif_id][kriteria_id] = sub_kriteria->nama
    $nilaiAlternatif = [];
    foreach ($penilaian as $p) {
        $nilaiAlternatif[$p->alternatif_id][$p->kriteria_id] = $p->subKriteria->nama ?? '-';
    }
    ...
    // Hitung totalKolom untuk tabel kriteria
    $totalKolom = [];
    foreach ($kriteria as $k) {
        $total = 0;
        foreach ($matriksNilaiKriteria as $item) {
            // Pastikan kolom dengan ID kriteria ada dan nilainya numerik
            if (isset($item->nilai) && $item->kriteria_id == $k->id) {
